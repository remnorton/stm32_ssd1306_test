...
static uint8_t init_sequence[] = {
							...
							//0xDA,0x22,  //set com pins hardware configuration (0x12)
...
uint8_t ssd1306_init(void* init)
{
	...
	if (!screen_buffer)
	{
		buf_size = _SSD1306_SCREEN_WIDTH_*_SSD1306_SCREEN_HEIGHT_/8;
...
void ssd1306_draw_bitmap(void* i, uint32_t x, uint32_t y, DSP_Bitmap_t* bmp, uint32_t color)
{
	SSD1306_INIT_t* instance = (SSD1306_INIT_t*)i;
	if (!instance) return;
    if (!bmp) return;
...
uint16_t ssd1306_draw_char(void* i, uint32_t x, uint32_t y, Font_type_t* font, uint8_t sym, uint32_t color)
{
	SSD1306_INIT_t* instance = (SSD1306_INIT_t*)i;
	if (!instance) return 0;
	if (!font) return 0;
	...
	if ( ((x+sym_width) > ssd1306_get_screen_width(i)) || ((y+font->height) > ssd1306_get_screen_height(i) )) return 0;
	...
	ssd1306_draw_bitmap(i,x,y+startY,&bmp,color);
	free(bmp.data);
